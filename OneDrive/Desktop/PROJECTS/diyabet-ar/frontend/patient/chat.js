...
(function(){
    ...
    try {
      ...
      if(response.ok){
        ...
        if(data.user.assignedDoctor){
          ...
          doctorName = data.user.assignedDoctor.fullName || 'طبيبك';
          document.getElementById('doctorName').textContent = doctorName;
          loadMessages();
        }
      }
    } catch(err){
      console.error('تعذر تحميل معلومات الطبيب', err);
    }
  }
  ...
  async function loadMessages(){
    ...
    try{
      const API_BASE = window.API_BASE || 'http://localhost:3001/api';
      const response = await fetch(API_BASE + `/patient/chat/${doctorId}`, {
        headers: { 'Authorization': 'Bearer ' + localStorage.getItem(AUTH_TOKEN_KEY) }
      });
      if(response.ok){
        const data = await response.json();
  ...
  async function sendMessage(){
    ...
    try{
      const API_BASE = window.API_BASE || 'http://localhost:3001/api';
      const response = await fetch(API_BASE + '/patient/chat/send', {
        method: 'POST',
        headers: {
  ...
  async function sendImage(){
    ...
    reader.onload = async function(e){
      try{
        ...
        console.error('تعذر إرسال الصورة', err);
      }
    };
