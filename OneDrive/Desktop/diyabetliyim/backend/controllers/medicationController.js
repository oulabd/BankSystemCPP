  ...
  try {
    ...
    if (doctorId) {
      await createNotification({
        user: doctorId,
        sender: req.user._id,
        type: NOTIFICATION_TYPES.RECORD_HIGH, // Reusing type, or create new MEDICATION type
        message: `${patient.fullName} logged ${medicationType}: ${name} (${dose} ${unit})`,
        link: `/doctor/medications.html?patientId=${req.user.id}`,
        meta: { medicationRecordId: medicationRecord._id }
      });
    }
...
async function recommendMedication(req, res) {
  try {
    ...
    await createNotification({
      ...
      message: `Your doctor recommended a new medication: ${name} (${dose} ${unit})`,
      link: `/patient/medications.html?recordId=${recommendation._id}`,
      meta: { medicationRecordId: recommendation._id }
    });
...
async function updateMedicationRecord(req, res) {
  try {
    ...
    await createNotification({
      ...
      sender: req.user._id,
      type: NOTIFICATION_TYPES.PRESCRIPTION_NEW,
      message: `Your doctor updated your medication: ${record.name}`,
      link: `/patient/medications.html?recordId=${record._id}`,
      meta: { medicationRecordId: record._id }
    });
