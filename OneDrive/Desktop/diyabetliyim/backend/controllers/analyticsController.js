...
function generateRecommendations(trends, riskFlags) {
  ...
  if (riskFlags.includes('morning_peaks')) {
    recommendations.push({
      type: 'warning',
      message: 'Consistent morning highs detected. Consider adjusting evening insulin or reducing bedtime snacks.',
      action: 'Consult with your doctor about basal insulin adjustment.'
    });
  }
  ...
  if (riskFlags.includes('night_hypoglycemia')) {
    recommendations.push({
      type: 'danger',
      message: 'Night-time low blood sugar detected. This can be dangerous.',
      action: 'Reduce evening insulin dose and have a small snack before bed. Contact your doctor immediately.'
    });
  }
  ...
  if (riskFlags.includes('frequent_highs')) {
    recommendations.push({
      type: 'warning',
      message: 'Frequent high readings detected.',
      action: 'Review your meal plan and insulin doses with your doctor.'
    });
  }
  ...
  if (riskFlags.includes('post_meal_peaks')) {
    recommendations.push({
      type: 'info',
      message: 'Post-meal spikes detected.',
      action: 'Consider taking rapid-acting insulin 15-20 minutes before meals.'
    });
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push({
      type: 'success',
      message: 'Your glucose levels are well controlled!',
      action: 'Keep up the good work and maintain your current routine.'
    });
  }
...
function generateInsulinRecommendations(overallControl, avgDoseResponse) {
  ...
  if (overallControl === 'needs_increase') {
    recommendations.push({
      type: 'warning',
      message: 'Your insulin doses may be insufficient.',
      action: 'Consult your doctor about increasing insulin doses. Average response is below target.',
      metric: `Average drop per unit: ${Math.round(avgDoseResponse)} mg/dL (Target: 30-50 mg/dL)`
    });
  } else if (overallControl === 'needs_decrease') {
    recommendations.push({
      type: 'danger',
      message: 'Your insulin doses may be too high, risk of hypoglycemia.',
      action: 'Contact your doctor immediately to review insulin doses.',
      metric: `Average drop per unit: ${Math.round(avgDoseResponse)} mg/dL (Target: 30-50 mg/dL)`
    });
  } else {
    recommendations.push({
      type: 'success',
      message: 'Your insulin doses are well calibrated.',
      action: 'Continue monitoring and maintain current regimen.',
      metric: `Average drop per unit: ${Math.round(avgDoseResponse)} mg/dL (Target: 30-50 mg/dL)`
    });
  }
