...
(function(){
  ...
  async function fetchLabs({start='', end='', type='' } = {}){
    ...
    const url = API_BASE + '/api/patient/labs' + (qsParts.length?('?'+qsParts.join('&')):'');
    try{
  ...
  async function createLab(formData){
    const token = localStorage.getItem(AUTH_TOKEN_KEY) || '';
    try{
      const res = await fetch(API_BASE + '/api/patient/labs', { method: 'POST', headers: { 'Authorization': 'Bearer ' + token }, body: formData });
  ...
  async function deleteLab(id){
    const token = localStorage.getItem(AUTH_TOKEN_KEY) || '';
    try{
      const res = await fetch(API_BASE + '/api/patient/labs/' + encodeURIComponent(id), { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token }});
  ...
  function createTable(items){
    ...
    thead.innerHTML = `
      <tr>
        <th data-i18n="labs.table.header.testName">Test Adı</th>
        <th data-i18n="labs.table.header.result">Sonuç</th>
        <th data-i18n="labs.table.header.refRange">Referans Aralığı</th>
        <th data-i18n="labs.table.header.date">Tarih</th>
        <th data-i18n="labs.table.header.notes">Notlar</th>
